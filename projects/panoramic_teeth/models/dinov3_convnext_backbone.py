...
class DINOv3ConvNextBackbone(BaseBackbone):
    """HuggingFace DINOv3 ConvNeXt backbone for panoramic teeth tasks."""
    ...
    def _validate_checkpoint_dir(self) -> None:
        ...
        if not checkpoint_dir.is_dir():
            raise FileNotFoundError(
                f'DINOv3 checkpoint directory does not exist: '
                f'{checkpoint_dir}')
    ...
    def _import_transformers():
        ...
        try:
            ...
            raise RuntimeError(
                ...
                'with `safetensors`. If `transformers` is already installed, '
                'verify it can be imported with the current PyTorch version.'
            ) from exc
    ...
    def _apply_trainable_stages(self) -> None:
        ...
        if missing_stages:
            raise RuntimeError(
                ...
                f'{missing_stages!r}. Check the underlying HuggingFace model '
                'structure before enabling fine-tuning.')
        ...
        for feature, expected_channels in zip(features, self.hidden_sizes):
            if feature.ndim != 4:
                raise RuntimeError(
                    ...
                    f'but got a tensor with shape {tuple(feature.shape)}.')
            if feature.shape[1] != expected_channels and feature.shape[-1] == expected_channels:
